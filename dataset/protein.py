    ...
    def process(self):
        ...
        for example_idx, class_idx in self.list_examples:
            ...
            if self.use_txt:
                ...
                protein.pos = torch.cat((protein.pos, txt_data), 1)
            if self.set_x == 1:
...
class InMemoryUnlabeledProteinSurfaceDataset(InMemoryDataset):
    def __init__(self, root, off_folder_path, txt_folder_path, args, num_examples=1543, final=False, transform=None, pre_transform=None):
        self.use_txt = args.use_txt
        self.final = final
        self.off_folder_path = off_folder_path
        self.txt_folder_path = txt_folder_path
        ...
        super(InMemoryUnlabeledProteinSurfaceDataset, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    
    @property
    def processed_file_names(self):
        return [f"unlabeled-data.pt"]

    def process(self):
        data_list = []
        for example_idx in range(1, self.num_examples + 1):
            off_path =  f"{self.off_folder_path}/{example_idx}.off" 
            txt_path =  f"{self.txt_folder_path}/{example_idx}.txt"
            protein = read_off(off_path)
            if self.use_txt:
